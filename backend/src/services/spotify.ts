...
// ─── Track scoring (rank top-5 results) ───────────────────────────────────────
type SpotifyTrack = {
  id: string;
  name: string;
  artists: { name: string }[];
...
function scoreTrack(track: SpotifyTrack, query: string): number {
  ...
  for (const a of track.artists) {
    if (q.includes(a.name.toLowerCase())) s += 20;
  }
  return s;
}
...
export class SpotifyService {
  ...
  public async findTrackUrl(songName: string): Promise<{url: string, previewUrl?: string} | null> {
    try {
      ...
      const items: SpotifyTrack[] | undefined = res.data?.tracks?.items;
   ...
   */
  public async getTrackInfoByUrl(url: string): Promise<{name: string, artists: string[]} | null> {
    const parts = url.split('/');
    let trackId = parts.pop()?.split('?')[0];
    if (trackId === 'track') trackId = parts.pop()?.split('?')[0]; // Handle /track/ID/
    ...
    try {
      ...
      if (token) {
        ...
        if (res.data && res.data.name) {
          return {
            name: res.data.name,
            artists: (res.data.artists || []).map((a: any) => a.name)
          };
        }
      }
    } catch (error) {
    ...
    try {
      const webRes = await axios.get(`https://open.spotify.com/track/${trackId}`, {
        headers: {
      ...
      if (titleMatch && titleMatch[1]) {
        ...
        if (m) {
          return {
            name: m[1].trim(),
            artists: m[2].split(',').map((a: string) => a.trim())
          };
        }
        ...
        if (m2) {
          return {
            name: m2[1].trim(),
            artists: m2[2].split(',').map((a: string) => a.trim())
          };
        }
